...
export class FridgeTagParser {
                ...
                if (isSection) {
                    if (key === Key.CONFIG) {
                        ...
                        this.contextStack.push({
                            indent,
                            section: Key.HISTORY,
                            obj: this.data.history,
                        })
                    } else if (key === Key.CERTIFICATE) {
                        this.contextStack.push({
                            indent,
                            section: Key.CERTIFICATE,
                            obj: this.data.certificate,
                        })
                    } else if (key === Key.ALARM) {
                        currentAlarmSection =
                            parentSection === Key.CONFIG ? 'config' : 'history'
                        this.contextStack.push({
                            indent,
                            section: Key.ALARM,
                            obj: null,
                        ...
                        if (parentSection === Key.HISTORY) {
                            ...
                            this.contextStack.push({
                                indent,
                                section: `Day${idx}`,
                                obj: currentHistRecord,
                            })
                        } else if (parentSection === Key.ALARM) {
                            this.contextStack.push({
                                indent,
                                section: `AlarmLevel${idx}`,
                                obj: idx,
                            })
                        }
                    }
    ...
    _setValue(section, key, value, histRecord, alarmSection) {
        ...
        if (section === 'root') {
            ...
            else if (key === Key.TEST_TIMESTAMP)
                this.data.config.testTimestamp = value
        } else if (
            section.startsWith('AlarmLevel') &&
            alarmSection === 'config'
        ) {
            const level = parseInt(section.replace('AlarmLevel', ''), 10)
            let threshold = this.data.config.alarmThresholds.find(
                (t) => t.level === level
            )
            if (!threshold) {
            ...
            else if (key === Key.DURATION_THRESHOLD)
                threshold.timeLimitMinutes = value
        } else if (section === Key.HISTORY) {
            if (key === Key.ACTIVATION_TIMESTAMP)
                this.data.history.activationTimestamp = value
            else if (key === Key.REPORT_CREATION_TIMESTAMP)
                this.data.history.reportCreationTimestamp = value
            ...
            else if (key === Key.MIN_TEMP_TIMESTAMP)
                histRecord.minTempTime = value
            else if (key === Key.MAX_TEMP) histRecord.maxTemp = value
            else if (key === Key.MAX_TEMP_TIMESTAMP)
                histRecord.maxTempTime = value
            else if (key === Key.AVG_TEMP) histRecord.avgTemp = value
            else if (key === Key.EVENTS) histRecord.events = value
        } else if (
            section.startsWith('AlarmLevel') &&
            alarmSection === 'history' &&
            histRecord
        ) {
            const level = parseInt(section.replace('AlarmLevel', ''), 10)
            ...
            if (key === Key.VERSION)
                this.data.certificate.version = String(value)
            else if (key === Key.LOT) this.data.certificate.lot = value
            else if (key === Key.ISSUER) this.data.certificate.issuer = value
            else if (key === Key.VALID_FROM)
                this.data.certificate.validFrom = value
            else if (key === Key.OWNER) this.data.certificate.owner = value
            else if (key === Key.PUBLIC_KEY)
                this.data.certificate.publicKey = value
        }
    }
}
            ...
            records: data.history.records.map((r) => ({
                ...
                alarms: r.alarms
                    ...
                    .map((a) => ({
                        ...
                        triggerTime:
                            a.timestamp !== '00:00' ? a.timestamp : null,
                        eventCount: a.count,
                    })),
